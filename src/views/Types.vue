  ...
  <div class="types">
    ...
    <v-expansion-panel>
      <v-expansion-panel-content>
        ...
        <NameButton v-for="poke_id in pokemons.concat(pokemons_alola).filter(member => member.type.includes($route.params.id)).sort(function(a,b){return (a.name[l_lang] >= b.name[l_lang] ? 1: -1);}).map(function(element){return element.id;})" :key="poke_id" :poke_id="poke_id" :lang="l_lang"/>
